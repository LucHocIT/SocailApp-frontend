...
import styles from './styles/CommentReactionStack.module.scss';

const CommentReactionStack = ({ 
  commentId, 
  reactionCounts, 
  maxVisible = 3,
  onClick 
  ...
  const getTooltipContent = () => {
    const reactionSummary = validReactions
      .map(([type, count]) => `${commentService.getReactionEmoji(type)} ${count}`)
      .join(', ');
        ...
        <div className={styles.reactionStack} onClick={handleClick}>
          <div className={styles.reactionEmojis}>
            {displayReactions.map(([type], index) => (
              ...
              >
                {commentService.getReactionEmoji(type)}
              </span>
            ))}
            {remainingCount > 0 && (
              <span 
...
export default CommentReactionStack;
